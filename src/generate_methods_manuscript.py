...
def generate_methods_manuscript():
    ...
    doc.add_paragraph(
        ...
        "this protocol, three of the four carriers show only modest, non-predictive physisorption "
        "(Q²_CV ≤ 0.1) — an honest but unglamorous result. The fourth, β12 borophene, chemisorbs 12 of 29 "
        "Tau-directed ligands through a new covalent bond, a finding that the field's common practice of a "
        "single, unrelaxed point calculation would have hidden entirely: our own first pass at this system made "
        "exactly that mistake, reporting an inert-looking physisorption energy for a chemically reactive "
        "surface. We use that mistake, corrected, as a worked example of what a relaxed-geometry, leak-free "
        "screen changes, and distill the broader lesson into a short checklist for computational drug-carrier "
        "screening."
    )
    ...
    doc.add_paragraph(
        "The common finding across three of the four systems — g-C3N4, MXene, and the boron nitride nanocage — "
        "is that no bare, pristine carrier gives a descriptor-based QSPR model of the adsorption energy that "
        "predicts meaningfully better than the training-set mean (Q²_CV ≤ 0.15 in every case, and "
        "indistinguishable from zero for two of the three). This is not a failure of the modelling: it is the "
        "honest signal that, for a physisorption-dominated interaction with these four descriptors, the "
        ...
        "scientific value of the screen, in these three cases, is the mechanistic picture the pipeline still "
        "delivers honestly — the interaction-energy regime, the charge-redistribution pattern, and, for "
        "g-C3N4, the effect of heteroatom doping — not a predictive surrogate model."
    )
    doc.add_paragraph(
        "The fourth system, β12 borophene against the Tau paired-helical-filament cohort, breaks this pattern "
        "outright, and is the subject of the next section."
    )
    ...
    doc.add_paragraph(
        ...
        "the honest outcome for three of our four disease systems is modest — non-predictive, physisorption-"
        "dominated interaction in the −10 to −40 kcal/mol range — and that modesty is itself useful information "
        "for anyone weighing whether to pursue a given carrier experimentally. For the fourth system, the same "
        "discipline reveals that the carrier is chemically reactive enough to covalently modify a third of the "
